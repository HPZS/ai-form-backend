...
// APIRevision 接口契约版本。请求体是严格解码(DisallowUnknownFields)的:
// 插件每加一个请求字段,存量后端就会对该能力整体报 400,用户看到的是无差别的行失败。
...
//
// rev5: MatchColumnsReq 增加 usedColumns(增量匹配时告诉模型哪些列已经有字段在填)。
//
//	2026-08-19 Shopee 实测:dynamic 语境只送新出现的字段,模型看不到已经建好的映射,
//	于是「数量」「价格」各被指派了两次——同一份数据填两个地方,交接卡上也就出现两条
//	一模一样的待办。这是**加请求字段**,老后端会整体 400,发布顺序照旧:后端先上线、插件后发。
//
// rev6: 新增能力 extract_reveal_chain(示范学习:从用户的操作序列里挑出"把隐藏字段点出来"的必要步骤)。
//
