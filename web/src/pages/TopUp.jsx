...
// 能力计费随后台配置变化，用户可查看实时能力价格表。
import React, { useContext, useEffect, useState } from 'react';
import { useSearchParams } from 'react-router-dom';
import CapabilityPricing from '../components/CapabilityPricing.jsx';
...
function baseBenefits(p, bonus) {
  return [
    ...
    ['订阅包含的 AI 能力不限次数', '具体能力以最新价格表中的「订阅包含」为准'],
    [`每月含 ${fmtNum(p.credits)} 积分`, '用于价格表中按次扣积分的 AI 能力'],
...
export default function TopUp() {
  ...
  return (
    <div className="topup">
      <CapabilityPricing open={pricingOpen} onClose={()=>setPricingOpen(false)} />
      <section className="topup-section">
        <header className="section-head">
          <h2>个人版订阅</h2>
          <Button onClick={()=>setPricingOpen(true)}>查看 AI 能力与积分价格</Button>
          <p>订阅解锁插件与自动化能力，AI 能力按最新配置享受订阅包含或按次扣积分</p>
        </header>
        ...
        <div className="spend-rules">
          <div><b>订阅包含</b><span>标记为订阅包含的能力不额外扣积分</span></div>
          <div><b>成功调用计费</b><span>按价格表中的单价结算；失败请求不扣积分</span></div>
          <div><b>本地复用不扣分</b><span>复用已有规则或结果无需调用 AI；重新生成属于新的调用</span></div>
        </div>
      </section>
